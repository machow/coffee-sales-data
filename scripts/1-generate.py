...
df["estimated_units"] = df.apply(
    lambda x: x["revenue_dollars"] / price_points[x["product"]], axis=1
)
...
def adjusted_monthly_sales(row):
    ...
    monthly_sales = [
        round(annual_sales * x / sum(varied_pattern)) for x in varied_pattern
    ]
